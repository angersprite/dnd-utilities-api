...
export default async function generateNPC(classID, raceID) {
    ...
    await Promise.all([
        goonrDAO.getRace(raceID) .then(r => { npc.race = r[0].name }),
        goonrDAO.getFirstName() .then(fn => { npc.firstName = fn  }),
        goonrDAO.getLastName().then(ln => { npc.lastName = ln  }),
        ...
        goonrDAO.getDescriptors(1) .then(desc => { npc.descriptors = desc }),
        goonrDAO.getSkills(classID, 1).then(skills => { npc.skills = skills }),
        goonrDAO.getItems(1).then(i => { npc.items = i  })
    ])
